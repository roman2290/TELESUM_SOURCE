#! C:\Users\User\Desktop\TELESUM_SOURCE\venv\Scripts\python.exe
...
API_ID = int(os.getenv("TELEGRAM_API_ID", ""))  # Преобразование в integer
...
# функция для чтеня чатов
async def read_chats():
    dialog = await client.get_dialogs()
    chats = [chats for chats in dialog if chats.is_user]
    for chat in chats:
        print(f'Чат с: {chat.name} (ID: {chat.id})')


# функция для запуска клиентов
async def main():
    await client.start(API_ID)
    print('Получаем данные чата')
    await read_chats()
...
async def new_message_handler(event):
    ...
    print(F'Новое сообщение в чате {chat.title if chat.title else chat.username}:')
...
async def new_handler(event):
    if event.is_private:
        ...
        user_id = sender.id
        await event.answer(f'Привет{username}, {user_id}')   
    else:
        await event.answer(f'Это не личный чат')



#Отправка сообщения в OpenAI
@client.on(events.NewMessage)
async def message_hendler(event):
    user_message = event.message.text

    # передаем сообщение пользователя
    ...
    await event.answer(bot_response)
...
def generate_openai_response(prompt):
    ...
    try:
        response = openai.ChatCompletion.create(
            ...
            max_tokens=5  # Максимальное количество токенов в ответе
        )
...
def validate_response(response):
    ...
    if len(response) < 10:  # Минимальная длина ответа
        return False, "Ответ слишком короткий."
    return True, "Ответ корректен."

def clean_response(response):
    """
    Очищает ответ от лишних символов и разметки.
    """
...
def save_response(response, file_path):
    ...
    try:
        with open(file_path, 'w', encoding='utf-8') as file:
            file.write(response)
        print(f"Ответ успешно сохранен в файл: {file_path}")
    except Exception as e:
        print(f"Ошибка при сохранении ответа: {e}")

# Пример использования
prompt = "Напишите краткое описание Python."
...
if response:
    ...
    if is_valid:
        ...
        save_response(cleaned_response, 'openai_response.txt')
    else:
        print(f"Ответ некорректен: {message}")
else:
...
async def new_mess_handler(event):
    """
    Обрабатывает новые сообщения и отправляет ответ с помощью OpenAI.
    """
    # Логируем входящее сообщение
    logger.info(f"Новое сообщение от {event.sender_id}: {event.text}")

    # Генерируем ответ с помощью OpenAI
    openai_response = generate_openai_response(event.text)

    if openai_response:
        # Логируем сгенерированный ответ
        logger.info(f"Сгенерированный ответ: {openai_response}")

        # Отправляем ответ обратно в чат
        await event.reply(openai_response)
        logger.info(f"Ответ отправлен в чат {event.chat_id}.")
    else:
